...
def search_clearml_task_artifacts_and_download_most_recent(input_task, artifact_name):
    ...
    for art_name in input_task.artifacts:
        ...
        if artifact_name == art_name:
            matching_artifact = input_task.artifacts[
                art_name
            ]  # clearml.binding.artifacts.Artifact has fields name, timestamp, etc.
...
def setup_pretrained_model_folder(
    pretrained_folder_path: str, clearml_input_task_id: str, needed_artifacts: list
):
    ...
    print(
        f"Downloading needed artifacts {needed_artifacts} from ClearML task with id={clearml_input_task_id} and name = {input_task.name}, moving them to {pretrained_folder_path}"
    )
    ...
    for needed_artifact in needed_artifacts:
        ...
        if desired_path.is_file():
            ...
            local_path = search_clearml_task_artifacts_and_download_most_recent(
                input_task, needed_artifact
            )
...
def clearml_task_setup(args):
    ...
    if args.clearml_input_task_id:
        setup_pretrained_model_folder(
            args.model_name_or_path,
            args.clearml_input_task_id,
...
def main():
    ...
    parser.add_argument(
        "--do_finetune", action="store_true", help="Whether to run training."
    )
    parser.add_argument(
    ...
    if args.do_finetune:
        tokenizer = tokenizer_class.from_pretrained(
            args.input_dir, do_lower_case=args.do_lower_case
        )
    ...
    if (args.do_train or args.do_finetune) and (
